...
<div style="background-color: #f8f8f8;padding:5px 0px;">
   <div class="container">
	 ...
	 <form action="${pageContext.request.contextPath}/manage/deviceimport.do" method="post"  enctype="multipart/form-data">
	   <div class="col-sm-4" style="position:relative">
	      <p style="margin-top:8px;width:80px;color:#5F7293;display:inline-block;">导入Excel</p>
	      <c:if test="${!empty emptyExcel}">
	         <span style="color:red;font-size:10px;">导入的设备信息为空,请重新选择文件</span>
	      </c:if>
	   ...
	   <div class="col-sm-4 text-center">
	       <p style="margin-top:8px;">导入设备信息</p>
	   </div>
  </div>
</div>
...
<div class="container" style="margin-top:10px;">
    <table class="table table-striped table-hover" style="background-color:#fff;">
       ...
       <c:if test="${!empty deviceDTOs}">
	       <c:forEach var="device" items="${deviceDTOs}">
	        <tr>
	           <td>${device.serialId }</td>
	           <td>${device.braceletNO }</td>
	           <td>${device.rfid }</td>
	           <td>${device.mac }</td>
	        </tr>
	        </c:forEach>
